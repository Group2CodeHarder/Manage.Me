...
class TaskActionButton extends React.Component {
  ...
  displayForm = () => {
      ...
      return (
        ...
        <textarea placeholder={placeholder} autoFocus onBlur={this.closeForm} value={this.state.text} onChange={this.handleChange} style={{marginLeft: "0", resize: "none", outline: "none", border: "none", overflow: "hidden", minHeight: "85", minWidth: "272" }}/>
        <div>
            <button onMouseDown={list ? this.handleAddList : this.handleAddCard} style={{color: "white", backgroundColor: "#87CEFA"}}>
                {buttonTitle}
            </button>
            <button style={{outline: "none", border: "none", backgroundColor: "inherit", marginLeft: "15px"}}>
                X
            </button>
        </div>
    </div>
